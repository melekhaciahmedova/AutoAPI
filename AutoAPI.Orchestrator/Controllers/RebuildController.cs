...
public class RebuildController(DockerService docker, ILogger<RebuildController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> RebuildApi()
    {
        ...
        // 1️⃣ Stop container
        steps.Add(await _docker.RunCommandAsync($"docker compose -f \"{composePath}/docker-compose.yml\" stop {serviceName}"));

        // 2️⃣ Remove container
        steps.Add(await _docker.RunCommandAsync($"docker compose -f \"{composePath}/docker-compose.yml\" rm -f {serviceName}"));

        // 3️⃣ Build new image
        steps.Add(await _docker.RunCommandAsync($"docker compose -f \"{composePath}/docker-compose.yml\" build {serviceName}"));

        // 4️⃣ Start container
        steps.Add(await _docker.RunCommandAsync($"docker compose -f \"{composePath}/docker-compose.yml\" up -d {serviceName}"));
